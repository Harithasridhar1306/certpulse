...
function checkTerminal(){
  const q=current.questions[index],spec=q[9]||{},raw=document.getElementById("manifest-editor").value.trim();
  if(!raw){showTerminal("Write your YAML first.","bad");return}
  const has=s=>raw.toLowerCase().includes(String(s).toLowerCase());
  const checks=[["apiVersion",/apiVersion\\s*:/i.test(raw)],["kind",has("kind: "+spec.kind)],["metadata.name",has("name: "+spec.name)]];
  const pairs={
    image:"image: ",port:"containerPort: ",targetPort:"targetPort: ",replicas:"replicas: ",
    selector:"",templateLabel:"",dataKey:"",dataValue:"",stringDataKey:"",stringDataValue:"",
    serviceType:"type: ",policyType:"",command:"",schedule:"",envKey:"",envValue:"",
    readinessPath:"",readinessPort:""
  };
  Object.keys(pairs).forEach(key=>{if(spec[key])checks.push([key,has(pairs[key]+spec[key])])});
  if(spec.denyAllIngress)checks.push(["deny-all ingress",/ingress\\s*:\\s*\\[\\s*\\]/i.test(raw)]);
  const passed=checks.filter(x=>x[1]).length,ok=passed===checks.length;
  answers[index]=ok?0:null;
  showTerminal((ok?"✓ Manifest checks passed. ":"Some required fields are missing. ")+passed+"/"+checks.length+" checks passed.",ok?"good":"bad");
}
